...
export type LeaveCertificateModel = {
  issueNo: string
  name: string
  birthYmd: string
  empId: string
...
export function makeLeaveIssueNo(): string {
  return makeCareerIssueNo()
}

export type LeaveCertificatePrintOpts = {
...
export function leaveCertificateDocumentHtml(m: LeaveCertificateModel, opts: LeaveCertificatePrintOpts): string {
  const todayKo = fmtKo(startOfDay(new Date()))
  const sealBlock = certificateSealHtml(opts.officerVerified)
  const period =
    m.leaveStart && m.leaveEnd
      ? `${escapeHtml(m.leaveStart)} ~ ${escapeHtml(m.leaveEnd)}`
      : escapeHtml(m.leaveStart || m.leaveEnd || '—')

  return `<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"/><title>휴직증명서</title>
  <style>
  ...
  <div class="t13" style="text-align:center;margin:16px 0 12px;">휴직증명서</div>
  <table class="meta t10">
    ...
    <tr><td>휴직기간</td><td>${period}</td></tr>
    <tr><td>휴직사유</td><td>${escapeHtml(m.leaveReason || '—')}</td></tr>
  </table>
