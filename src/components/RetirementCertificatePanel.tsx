...
import { type GeminiRetirementRecordPayload } from '../lib/careerGemini'
import {
  retirementCertificateDocumentHtml,
  makeRetirementIssueNo,
  type RetirementCertificateModel,
} from '../lib/retirementCertificate'
import { parseCareerRecordFromBuffer, buildCareerCertificateModelFromParsed } from '../lib/careerRecordExcel'
import {
  downloadTextFile,
  exportCareerPdfToFile,
  geminiAnalyzeRetirementRecord,
  isDesktopApp,
...
export function RetirementCertificatePanel() {
  const desktop = isDesktopApp()
  const [filePath, setFilePath] = useState<string | null>(null)
  const [fileName, setFileName] = useState<string | null>(null)
  const [empId, setEmpId] = useState('')
  const [jobType, setJobType] = useState<(typeof JOB_TYPES)[number]>('기술직')
  const [model, setModel] = useState<RetirementCertificateModel | null>(null)
  ...
  const onIssue = useCallback(async () => {
    ...
    try {
      if (isPdfPath(filePath)) {
        const r = await geminiAnalyzeRetirementRecord({ filePath, empId: empId.trim() })
        if (!r.ok) {
          setStatusMsg(r.message || '분석 실패')
          return
        }
        const rec = r.record as GeminiRetirementRecordPayload
        setModel({
          issueNo: makeRetirementIssueNo(),
          name: String(rec.name ?? '').trim(),
          birthYmd: String(rec.birthYmd ?? '').trim(),
          empId: String(rec.empId ?? empId).trim() || empId.trim(),
          hireYmd: String(rec.hireYmd ?? '').trim(),
          retireYmd: String(rec.retireYmd ?? '').trim(),
        ...
        setModel({
          ...
          empId: base.empId,
          hireYmd: base.hireYmd,
          retireYmd: '',
          jobType: base.jobType,
          rankLabel: base.rankLabel,
          warnings: [
            ...base.warnings,
            '엑셀에서는 퇴직(예정)일을 직접 입력해 주세요. PDF는 Gemini로 자동 추출합니다.',
          ],
        })
      } else {
  ...
  const onOfficerChange = useCallback((checked: boolean) => {
    ...
    if (checked) setOfficialIssueNo(makeRetirementIssueNo())
  ...
  const saveDocument = useCallback(async () => {
    ...
    setSaving(true)
    try {
      const issueNo = officerChecked ? (officialIssueNo ?? makeRetirementIssueNo()) : 'TEST'
      if (officerChecked && !officialIssueNo) setOfficialIssueNo(issueNo)
      const html = retirementCertificateDocumentHtml(model, {
        officerVerified: officerChecked,
        issueNo,
      })
      if (isDesktopApp()) {
        const r = await exportCareerPdfToFile(html, `퇴직예정증명서-${model.empId}.pdf`)
        if (r.canceled) return
        if (r.ok && r.filePath) window.alert(`저장했습니다.\n${r.filePath}`)
      } else {
        downloadTextFile(`퇴직예정증명서-${model.empId}.html`, html, 'text/html;charset=utf-8')
      }
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-slate-600">
        증빙폴더의 인사기록부로 퇴직(예정)증명서를 작성합니다. PDF는 Gemini가 퇴직·입사 정보를 추출합니다.
      </p>
...
function RetirementPreview({
  ...
  return (
    <div className="mx-auto max-w-[640px] text-black" style={{ fontFamily: '"Malgun Gothic",sans-serif' }}>
      ...
      <div className="py-3 text-center text-[13pt] font-bold">퇴직(예정)증명서</div>
