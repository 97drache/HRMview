...
function attachProofStyles(fontFace) {
  return `
  ${fontFace}
  ${pageBaseCss()}
  body {
...
/** 1페이지 증빙서 붙임란 양식만 (추가 첨부는 PDF 병합으로 처리) */
function buildAttachProofHtml(fields, receiptRel, fontRelPath) {
...
<meta charset="utf-8" />
<title>증빙서 붙임란</title>
<style>${attachProofStyles(fontFace)}</style>
</head>
<body>
  <div class="sheet page-break">
    ...
    <div class="attach-main">
      <img src="${escapeHtml(receiptRel)}" alt="영수증" />
    </div>
...
module.exports = { buildAttachProofHtml, SHEET_WIDTH_MM }
